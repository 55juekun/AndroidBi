...
public class AliyunVodPlayerView extends RelativeLayout implements ITheme {
    ...
    //自定义的截图按钮，录制视频按钮点击监听
    private ControlView.OnCutClickListener mOnCutClickListener;
    private ControlView.OnRecordClickListener mOnRecordClickListener;
...
//先隐藏手势和控制栏，防止在没有prepare的时候做操作。
    ...
    private void initControlView() {
        ...
        mControlView.setOnDownloadClickListener(new OnDownloadClickListener() {
            ...
            public void onDownloadClick() {
                ...
                new DownloadMethod(getContext(), AliyunPlayerSkinActivity.id, AliyunPlayerSkinActivity.currentPlayUrl);
            }
        });
        //清晰度按钮点击
        ...
        mControlView.setOnCutClickListener(new ControlView.OnCutClickListener() {
            @Override
            public void onClick() {
                if ((mOnCutClickListener != null)) {
                    mOnCutClickListener.onClick();
                }
            }
        });
        mControlView.setOnRecordClickListener(new ControlView.OnRecordClickListener() {
            @Override
            public void onClick() {
                if ((mOnRecordClickListener != null)) {
                    mOnRecordClickListener.onClick();
                }
            }
        });
    }
        ...
        mSpeedView.setOnSpeedClickListener(new SpeedView.OnSpeedClickListener() {
            ...
            public void onSpeedClick(SpeedView.SpeedValue value) {
                ...
                if (value == SpeedView.SpeedValue.Normal) {
                    speed = 1.0f;
                } else if (value == SpeedView.SpeedValue.Quartern) {
                    speed = 0.25f;
                } else if (value == SpeedView.SpeedValue.Half) {
                    speed = 0.5f;
                } else if (value == SpeedView.SpeedValue.Twice) {
                    speed = 2.0f;
                }else if (value == SpeedView.SpeedValue.Four) {
                    speed = 4.0f;
                }else if (value == SpeedView.SpeedValue.Eight) {
                    speed = 8.0f;
                }
    ...
    /**
     * 自定义截图，录制
     * 简述接口调用流程：1.ControlView定义出接口及相关调用方法，并在layout内使用某个组件绑定该接口
     *                  2.AliyunVodPlayerView引用ControlView的相关接口，起中间传递的功能
     *                  3.AliyunPlayerSkinActivity Override ControlView 中的接口，并具体实现该接口的功能
     *                  4.AliyunPlayerSkinActivity 将接口设置于AliyunVodPlayerView，从而实现绑定功能与组件
     *                  优势在于分离组件与功能，某些接口的方法只能在Activity内实现
     */
    public interface OnCutClickListener{
        void onClick();
    }
    public void setOnCutClickListener(ControlView.OnCutClickListener listener){
        this.mOnCutClickListener=listener;
    }

    public interface OnRecordClickListener{
        void onClick();
    }
    public void setOnRecordClickListener(ControlView.OnRecordClickListener listener){
